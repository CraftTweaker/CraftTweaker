...
public final class MCRecipeManager implements IRecipeManager {
    ...
    public static class SubActionReplaceAllOccurences implements IAction {
        ...
        private IIngredient changeIngredient(@Nullable IIngredient input) {
            if (input == null) {
                return toReplace == MCItemStack.EMPTY ? replaceWith : null;
            } else {
                List<IItemStack> inputItems = input.getItems();
                if (inputItems == null) return input;
        ...
        private void applyShapelessPattern() {
            if (ActionReplaceAllOccurences.INSTANCE.ingredients1D != null) {
                ...
                if (replaceWithAir) {
                    ActionReplaceAllOccurences.INSTANCE.ingredients1D = ArrayUtils.removeAllOccurences(ActionReplaceAllOccurences.INSTANCE.ingredients1D, null);
                }
            }
        }
