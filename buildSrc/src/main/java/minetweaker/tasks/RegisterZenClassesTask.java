...
public class RegisterZenClassesTask extends DefaultTask {
    ...
    @TaskAction
    public void doTask() {
        System.out.println("registering Zen classes");
        if(outputDir == null)
            outputDir = inputDir;
    ...
    private void processJavaClass(File cls, String pkg, List<String> classNames, List<OnRegisterMethod> onRegisterMethods) {
        try {
            ...
            for(MethodAnnotationDetector onRegisterMethod : detector.onRegister) {
                onRegisterMethods.add(new OnRegisterMethod(pkg + "/" + cls.getName().substring(0, cls.getName().length() - 6), onRegisterMethod.name));
            }
        } catch(IOException ex) {
    ...
    private static class AnnotationDetector extends ClassVisitor {
        ...
        public AnnotationVisitor visitAnnotation(String desc, boolean visible) {
            ...
            if(desc.equals("Lstanhebben/zenscript/annotations/ZenClass;")) {
                isAnnotated = true;
            }
            if(desc.equals("Lminetweaker/annotations/BracketHandler;")) {
                isAnnotated = true;
            }
