...
import minetweaker.mc11.player.MCPlayer;
...
public class MCCraftingInventory implements ICraftingInventory {
		...
		for(int i = 0; i < inventory.getSizeInventory(); i++) {
			if(changed(i)) {
				...
				if(!inventory.getStackInSlot(i).isEmpty()) {
					if(stacks[i] == null || ((ItemStack) stacks[i].getInternal()).isEmpty())
						stackCount++;
					stacks[i] = getIItemStack(original[i]);
				} else {
					if(stacks[i] != null && !((ItemStack) stacks[i].getInternal()).isEmpty())
						stackCount--;
					stacks[i] = getIItemStack(ItemStack.EMPTY);
				}
			}
		}
	...
	private boolean changed(int i) {
		return original[i] != inventory.getStackInSlot(i) || original[i] != null && stacks[i].getAmount() != original[i].getCount();
