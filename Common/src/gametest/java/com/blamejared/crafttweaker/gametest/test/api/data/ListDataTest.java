...
public class ListDataTest implements CraftTweakerGameTest {
    
    private String named(String name) {
        
        return "data/list/%s.zs".formatted(name);
    }
    ...
    public void testConstructor(GameTestHelper helper, ScriptBuilder builder) {
        ...
        assertThat(iData, is(new ListData(new IData[] {new StringData("first"), new StringData("second")})));
    }
    ...
    public void testToList(GameTestHelper helper, ScriptBuilder builder) {
        ...
        log.assertNoErrors();
        log.assertNoWarnings();
        log.assertOutput(0, "first");
        log.assertOutput(1, "second");
        
        IData iData = GameTestGlobals.data().get(0);
        
        assertThat(iData.asList(), is(List.of(new StringData("first"), new StringData("second"))));
    }
    ...
    public void testEmpty(GameTestHelper helper, ScriptBuilder builder) {
        ...
        assertThat(iData.asList().size() == 0, is(false));
    }
    ...
    public void testIterator(GameTestHelper helper, ScriptBuilder builder) {
        ...
        assertThat(iterator.next().getAsString(), is("first"));
        assertThat(iterator.next().getAsString(), is("second"));
    }
