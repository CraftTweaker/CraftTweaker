...
@ZenRegister
@ZenCodeType.Name("crafttweaker.api.custom_commands.MCRedirectModifier")
public class MCRedirectModifier {
    ...
    public MCRedirectModifier(Function<MCCommandContext, Collection<MCCommandSource>> fun) {
        internal = context -> fun.apply(new MCCommandContext(context))
                .stream()
                .map(MCCommandSource::getInternal)
    ...
    public Collection<MCCommandSource> apply(final MCCommandContext context) throws Exception {
        return internal.apply(context.getInternal()).stream()
                .map(MCCommandSource::new)
                .collect(Collectors.toList());
    }
