...
@ZenRegister
@ZenCodeType.Name("crafttweaker.api.custom_commands.MCCommandDispatcher")
public class MCCommandDispatcher {
    ...
    public Map<MCCommandNode, String> getSmartUsage(final MCCommandNode node, final MCCommandSource source) {
        return internal.getSmartUsage(node.getInternal(), source.getInternal()).entrySet()
                .stream()
    ...
    public void findAmbiguities(final MCAmbiguityConsumer consumer) {
        internal.findAmbiguities((parent, child, sibling, inputs) -> consumer.ambiguous(MCCommandNode.convert(parent), MCCommandNode
                .convert(child), MCCommandNode.convert(sibling), inputs));
    }
