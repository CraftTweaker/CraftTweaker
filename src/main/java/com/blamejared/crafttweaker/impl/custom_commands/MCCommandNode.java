...
@ZenRegister
@ZenCodeType.Name("crafttweaker.api.custom_commands.MCCommandNode")
public class MCCommandNode {
    ...
    public Collection<MCCommandNode> getChildren() {
        return internal.getChildren().stream()
                .map(MCCommandNode::new)
    ...
    public Collection<MCCommandNode> getRelevantNodes(final StringReader input) {
        return internal.getRelevantNodes(input).stream()
                .map(MCCommandNode::new)
                .collect(Collectors.toList());
    }
