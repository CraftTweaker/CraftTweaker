...
import minetweaker.mc164.block.MCWorldBlock;
...
public class MineTweakerMC {
	...
	public static int getBlockId(IBlockDefinition blockDefinition) {
		if (blockDefinition instanceof MCBlockDefinition) {
			return ((MCBlockDefinition) blockDefinition).getInternalId();
		} else {
			throw new IllegalArgumentException("block definition is not generated by MineTweaker");
		}
	}
	
	public static ItemStack toItemStack(IBlock block) {
		int mcBlockId = getBlockId(block.getDefinition());
		ItemStack result = new ItemStack(mcBlockId, 1, block.getMeta());
