...
public final class TypePage extends DocumentationPage {
    ...
    public void fillMeta(DocumentMeta meta) {
        ...
        Set<TypePageMeta> implementedTypesMeta = new HashSet<>();
        if(superType instanceof TypePageTypeInfo){
            TypePageMeta superTypeMeta = new TypePageMeta();
            superTypeMeta.setZenCodeName(((TypePageTypeInfo) superType).getZenCodeName().getZenCodeName());
            implementedTypesMeta.add(superTypeMeta);
        }
        if(!implementedTypes.isEmpty()){
            for(AbstractTypeInfo implementedType : implementedTypes) {
                if(implementedType instanceof TypePageTypeInfo){
                    TypePageMeta interfaceMeta = new TypePageMeta();
                    interfaceMeta.setZenCodeName(((TypePageTypeInfo) implementedType).getZenCodeName().getZenCodeName());
                    implementedTypesMeta.add(interfaceMeta);
                }
            }
        }
        meta.setImplementedTypes(implementedTypesMeta);
    }
