...
public class MCWorldBlock implements IBlock {
	...
	public IBlockDefinition getDefinition() {
		return MineTweakerMC.getBlockDefinition(blocks.getBlockState(position).getBlock());
	}

	@Override
	public int getMeta() {
		return blocks.getBlockState(position).getBlock().getMetaFromState(blocks.getBlockState(position));
	}

	@Override
	public IData getTileData() {
		TileEntity tileEntity = blocks.getTileEntity(position);
	...
	public String getDisplayName() {
		Block block = blocks.getBlockState(position).getBlock();
