...
public class AssemblyTable {
	...
	public static void removeRecipe(IIngredient output, @Optional IIngredient[] ingredients, @Optional boolean wildcard) {
		...
		outer: for (AssemblyRecipe recipe : AssemblyRecipe.assemblyRecipes) {
			if (output.matches(MineTweakerMC.getIItemStack(recipe.output))
	...
	private static boolean ingredientsMatch(AssemblyRecipe recipe, IIngredient[] ingredients, boolean wildcard) {
		...
		checkIngredient: for (int i = 0; i < recipe.input.length; i++) {
			Object ingredientObject = recipe.input[i];
			if (ingredientObject instanceof Number)
				continue;
			
			IIngredient recipeIngredient = MineTweakerMC.getIIngredient(recipe.input[i]);
	...
	private static class AddRecipeAction implements IUndoableAction {
		...
		public AddRecipeAction(IItemStack output, double energy, IIngredient[] ingredients) {
			...
			recipe = new AssemblyRecipe((int) energy, MineTweakerMC.getItemStack(output), mcIngredients);
		}
		...
		@Override
		public void undo() {
			AssemblyRecipe.assemblyRecipes.remove(recipe);
		}
	...
	private static class RemoveRecipeAction implements IUndoableAction {
		...
		@Override
		public String describe() {
			return "Removing assembly table recipe for " + MineTweakerMC.getIItemStack(recipe.output);
		}

		@Override
		public String describeUndo() {
			return "Restoring assembly table recipe for " + MineTweakerMC.getIItemStack(recipe.output);
		}
