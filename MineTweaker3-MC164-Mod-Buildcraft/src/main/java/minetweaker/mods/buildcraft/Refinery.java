...
public class Refinery {
	...
	static {
		...
		try {
			constructor = RefineryRecipes.Recipe.class.getDeclaredConstructor(FluidStack.class, FluidStack.class, FluidStack.class, int.class, int.class);
		...
		Field recipes = null;
		try {
			recipes = RefineryRecipes.class.getDeclaredField("recipes");
	...
	public static void remove(ILiquidStack output) {
		...
		List<RefineryRecipes.Recipe> toRemove = new ArrayList<RefineryRecipes.Recipe>();
		for (RefineryRecipes.Recipe recipe : RefineryRecipes.getRecipes()) {
			if (recipe.result.getFluid() == fluid) {
				toRemove.add(recipe);
			}
		}
		
		for (RefineryRecipes.Recipe recipe : toRemove) {
			MineTweakerAPI.apply(new RemoveRecipeAction(recipe));
		}
	}
	
	private static SortedSet<RefineryRecipes.Recipe> getRecipes() {
		try {
			return (SortedSet<RefineryRecipes.Recipe>) REFINERYRECIPEMANAGER_RECIPES.get(null);
	...
	private static class AddRecipeAction implements IUndoableAction {
		...
		public AddRecipeAction(ILiquidStack output, int energyPerMB, int ticksPerMB, ILiquidStack input1, ILiquidStack input2) {
			...
			RefineryRecipes.Recipe rrecipe = null;
			try {
				rrecipe = (RefineryRecipes.Recipe) CONSTRUCT_REFINERYRECIPE.newInstance(
	...
	private static class RemoveRecipeAction implements IUndoableAction {
		...
		public RemoveRecipeAction(RefineryRecipes.Recipe recipe) {
			this.recipe = recipe;
		}
		...
		public String describe() {
			return "Removing refinery recipe for "
					+ recipe.result.getFluid().getLocalizedName();
		}
